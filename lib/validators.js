...
export const validateNombre = (value, maxLength = 50) => {
  // Aseguramos que maxLength sea tratado como un número, o usamos 50 como fallback.
  // Esto previene que si se le pasa 'formData' como segundo argumento, falle.
  const maxLen = typeof maxLength === 'number' ? maxLength : 50;

  if (!value || !value.trim()) {
    return "Este campo es obligatorio";
  }
  
  // Usamos maxLen en lugar de maxLength
  const regex = new RegExp(`^[A-Za-zÁÉÍÓÚáéíóúÑñ\\s]{1,${maxLen}}$`);
  if (!regex.test(value)) {
    return `Solo letras y espacios (máx ${maxLen} caracteres)`;
  }
